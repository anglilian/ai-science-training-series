...
parallel_threads_list = [128, 178, 228] 
# This controls how many batches to prefetch
prefetch_buffer_size_list = [8, 10,12] # tf.data.AUTOTUNE
...
# record data to plot
plot_mean_rate = np.zeros((len(parallel_threads_list), len(prefetch_buffer_size_list)))
plot_stddev_rate = np.zeros((len(parallel_threads_list), len(prefetch_buffer_size_list)))
...
def train_epoch(i_epoch, step_in_epoch, train_ds, val_ds, network, optimizer, BATCH_SIZE, parallel_threads, prefetch_buffer_size, checkpoint):
    ...
    if use_profiler:
        ...
        tf.profiler.experimental.start('logdir/m%03d_w%02d_p%02d' % (parallel_threads,parallel_threads,prefetch_buffer_size))
...
def prepare_data_loader(BATCH_SIZE, parallel_threads, prefetch_buffer_size):
...
def main():
    ...
    for i, parallel_threads in enumerate(parallel_threads_list):
        os.environ['OMP_NUM_THREADS'] = str(parallel_threads)
        
        for k, prefetch_buffer_size in enumerate(prefetch_buffer_size_list):
            
            train_ds, val_ds = prepare_data_loader(BATCH_SIZE, parallel_threads, prefetch_buffer_size)
            ...
            while epoch < N_EPOCHS:
                mean_rate, stddev_rate = train_epoch(epoch, step_in_epoch, train_ds, val_ds, network, optimizer, BATCH_SIZE, parallel_threads, prefetch_buffer_size, checkpoint)
                epoch.assign_add(1)
                step_in_epoch.assign(0)
                
                plot_mean_rate[i][k] = mean_rate
                plot_stddev_rate[i][k] = stddev_rate
           
    plt.imshow(plot_mean_rate)
    plt.xlabel("Parallel threads")
    plt.ylabel("Prefetch buffer size")
    plt.colorbar()
    plt.savefig("mean_rate.png")
    
    plt.imshow(plot_stddev_rate)
    plt.xlabel("Parallel threads")
    plt.ylabel("Prefetch buffer size")
    plt.colorbar()
    plt.savefig("stddev_rate.png")
